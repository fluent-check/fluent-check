...
describe('Arbitrary tests', () => {
  ...
  describe('Can Generate', () => {
    ...
    it('honors shrinkHelper on constant.map()', () => {
      // Without shrinkHelper, constant.map returns a simple ArbitraryConstant
      const simpleMap = fc.constant(5).map(x => x * 2)
      expect(simpleMap.canGenerate({value: 10, original: 5})).to.be.true

      // With custom canGenerate, it should use the provided function
      const withCanGenerate = fc.constant(5).map(
        x => x * 2,
        {canGenerate: pick => pick.value === 10}
      )
      expect(withCanGenerate.canGenerate({value: 10, original: 5})).to.be.true
      expect(withCanGenerate.canGenerate({value: 10, original: 999})).to.be.true  // custom canGenerate ignores original

      // With inverseMap, it should use it for canGenerate
      const withInverseMap = fc.constant(5).map(
        x => x * 2,
        {inverseMap: b => [b / 2]}
      )
      expect(withInverseMap.canGenerate({value: 10, original: 5})).to.be.true
      expect(withInverseMap.canGenerate({value: 12, original: 5})).to.be.false  // 12/2 = 6 != 5
    })
