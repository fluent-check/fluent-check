...
describe('Arbitrary tests', () => {
  ...
  describe('Can Generate', () => {
    ...
    it('knows if it can generate a string', () => {
      ...
      expect(fc.string(2, 4, 'abcd').canGenerate({ value: 'a' })).to.be.false
    })
    ...
    it('knows if it can generate an array', () => {
      expect(fc.array(fc.integer(1, 10), 1, 10).canGenerate({ value: [1, 2, 3] })).to.be.true
      expect(fc.array(fc.integer(1, 10), 1, 10).canGenerate({ value: [1, 2, 30] })).to.be.false
      expect(fc.array(fc.integer(1, 2), 1, 10).canGenerate({ value: [1, 2, 3] })).to.be.false
    })
    ...
    it('knows if it can be generated by a map', () => {
      expect(fc.integer(0,4).map(n => String.fromCharCode(n + 97)).canGenerate({ original: 0, value: 'a' })).to.be.true
      expect(fc.integer(0,4).map(n => String.fromCharCode(n + 97)).canGenerate({ original: 2, value: 'c' })).to.be.true
      expect(fc.integer(0,4).map(n => String.fromCharCode(n + 97)).canGenerate({ original: 3, value: 'c' })).to.be.false
      expect(fc.integer(0,4).map(n => String.fromCharCode(n + 97)).canGenerate({ original: 5, value: 'f' })).to.be.false
    })
