...
describe('Arbitrary tests', () => {
  ...
  it('should allow shrinking of mapped tupples', () => {
    expect(fc.scenario()
      .exists('point', fc.tuple(
        ...
        fc.string(1, 10, fc.char(97,97)).filter(x => x.length > 2)).map(([a, b]) => [a * 2, '_'.concat(b)]))
  ...
  describe('Corner Cases', () => {
    ...
    it('should return the corner cases of strings', () => {
      ...
      expect(fc.string(1, 3, fc.char(97,97)).cornerCases().map(c => c.value)).to.have.members(['a', 'aaa'])
      expect(fc.string(1, 3, fc.char(97,98)).cornerCases().map(c => c.value)).to.have.members(['a', 'aaa', 'b', 'bbb'])
      expect(fc.string(1, 3, fc.char(97,99)).cornerCases().map(c => c.value)).to.have.members(
        ['a', 'aaa', 'b', 'bbb', 'c', 'ccc']
      )
      expect(fc.string(1, 3, fc.char(97,100)).cornerCases().map(c => c.value)).to.have.members(
        ['a', 'aaa', 'c', 'ccc', 'd', 'ddd']
      )
      expect(fc.string(1, 3, fc.char(97,101)).cornerCases().map(c => c.value)).to.have.members(
        ['a', 'aaa', 'c', 'ccc', 'e', 'eee']
      )
    })
    ...
    it('should return the corner cases of tuples', () => {
      expect(fc.tuple(fc.integer(0, 1), fc.string(1, 2, fc.char(97,99)))
  ...
  describe('Can Generate', () => {
    ...
    it('knows if it can be generated by a tuple', () => {
      expect(fc.tuple(fc.integer(1, 5), fc.string(1, 5, fc.char(97,99)))
