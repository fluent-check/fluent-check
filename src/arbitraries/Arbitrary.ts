...
import {splitmix64, stringify} from './util'

export abstract class Arbitrary<A> {
  /**
   * This variable contains the information used for random element generation.
   */
  gen: {
    state: number[]
    seed: number
  ...
  /**
   * Arbitrary class constructor which generates the seed used for random element generation.
   * For seed generation, splitmix64 is used as instructed by the creator of xoroshiro.
   */
  constructor() {
    const x: {val: number} = {val: Math.floor(Math.random() * 0x10000)}
    for (let i = 0; i < 2; i++) {
      this.gen.state[i] = splitmix64(x)
      this.gen.seed += this.gen.state[i] << (i*64)
    }
  }
