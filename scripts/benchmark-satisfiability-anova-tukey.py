...
def time(df):
    ...
    filteredDf = df.filter(items=['Strategy', 'Mean Time (ms)'])
    
    data = []
    for c in config:
        for k in mixins.keys():
            with_ = filteredDf[filteredDf['Strategy'].isin(withMixin(mixins[k], c))]['Mean Time (ms)'].values.tolist()
            without_ = filteredDf[filteredDf['Strategy'].isin(withoutMixin(mixins[k], c))]['Mean Time (ms)'].values.tolist()
    ...
    if len(data) > 0:
        pd.DataFrame(data=data, columns=COLUMNS).to_csv(PATH + p.split('.')[0] + '_TIME.csv', index=False)

def coverage(df):
    c = 'Coverage-Guided'
    mixins = {'Bias': BIAS, 'PairWiseTesting': PAIR_WISE, 'ConstantExtraction': CONSTANT_EXTRACTION}
    filteredDf = df.filter(items=['Strategy', 'Mean Coverage (%)'])

    data = []
    for k in mixins.keys():
        with_ = filteredDf[filteredDf['Strategy'].isin(withMixin(mixins[k], c))]['Mean Coverage (%)'].values.tolist()
        without_ = filteredDf[filteredDf['Strategy'].isin(withoutMixin(mixins[k], c))]['Mean Coverage (%)'].values.tolist()
    ...
    if len(data) > 0:
        pd.DataFrame(data=data, columns=COLUMNS).to_csv(PATH + p.split('.')[0] + '_COVERAGE.csv', index=False)

def testCases(df):
    config = ['Random', 'Coverage-Guided']
    mixins = {'Bias': BIAS, 'PairWiseTesting': PAIR_WISE, 'ConstantExtraction': CONSTANT_EXTRACTION}
    filteredDf = df.filter(items=['Strategy', 'Mean Test Cases'])

    data = []
    for c in config:
        for k in mixins.keys():
            with_ = filteredDf[filteredDf['Strategy'].isin(withMixin(mixins[k], c))]['Mean Test Cases'].values.tolist()
            without_ = filteredDf[filteredDf['Strategy'].isin(withoutMixin(mixins[k], c))]['Mean Test Cases'].values.tolist()
    ...
    for k in mixins.keys():
        with_R  = filteredDf[filteredDf['Strategy'].isin(withMixin(mixins[k], 'Random'))]['Mean Test Cases'].values.tolist()
        with_CG = filteredDf[filteredDf['Strategy'].isin(withMixin(mixins[k], 'Coverage-Guided'))]['Mean Test Cases'].values.tolist()
    ...
    R  = filteredDf[filteredDf['Strategy'].isin(RANDOM)]['Mean Test Cases'].values.tolist()
    CG = filteredDf[filteredDf['Strategy'].isin(COVERAGE_GUIDED)]['Mean Test Cases'].values.tolist()
    ...
    if len(data) > 0:
        pd.DataFrame(data=data, columns=COLUMNS).to_csv(PATH + p.split('.')[0] + '_TEST_CASES.csv', index=False)
