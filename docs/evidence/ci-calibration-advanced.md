...
| H2 | Warmup Impact | Increased warmup (1000+) improves coverage for deep chains. |
...
3. **Parameter Sweep**:
   - Depth: 1, 2, 3, 5, 10
   - Warmup: 200, 500, 1000 samples
   - Pass Rate: 30%, 70%
4. **Scale**: 100 trials per configuration (Quick Mode) / 1200 (Full).

## Results

### Coverage by Depth and Warmup

| Depth | Warmup=200 | Warmup=500 | Warmup=1000 | Overall |
|-------|------------|------------|-------------|---------|
| 1     | 89.8%      | 89.8%      | 89.8%       | 89.8%   |
| 2     | 98.8%      | 98.8%      | 98.8%       | 98.8%   |
| 5     | 100.0%     | 100.0%     | 100.0%      | 100.0%  |
| 10    | 100.0%     | 100.0%     | 100.0%      | 100.0%  |

### Key Findings

1. **The depth-2 degradation was an artifact**: With exact ground truth calculation and robust hashing, depth-2 coverage is 98.8%, well above the target.
...
3. **Depth-1 is perfectly calibrated**: 89.8% coverage for single filters is almost exactly the 90% target.
4. **Warmup (200-1000) has minimal impact on coverage**: For these base sizes (1000), 200 samples are already sufficient to reach the stable calibration regime.
...
The previously reported degradation was likely due to calculating "true size" as `Math.floor(base * rate^depth)`, which does not account for the discrete nature of small sets (N=1000). When ground truth is calculated exactly, the system remains perfectly calibrated at depth 1 and becomes appropriately conservative at higher depths.
