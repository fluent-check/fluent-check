...
- [ ] 1.1 Add `ExactSizeArbitrary<A>` interface to `types.ts`
- [ ] 1.2 Add `EstimatedSizeArbitrary<A>` interface to `types.ts`
...
- [ ] 2.3 Add method overloads for `map()` in interface definitions
...
- [ ] 5.3 Add tests verifying `map()` preserves size type
- [ ] 5.4 Run `npm run test:types` and verify all tests pass
