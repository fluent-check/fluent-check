...
The system SHALL extend FluentStatistics to support detailed statistics.

#### Scenario: Backward compatibility
- **WHEN** detailed statistics are disabled (default)
- **THEN** `FluentStatistics` SHALL behave identically to the current implementation
- **AND** all existing fields SHALL remain unchanged
- **AND** new fields SHALL be optional (`undefined` when not enabled)

#### Scenario: Type definitions
- **WHEN** the FluentStatistics interface is extended
